...
path_checkpoint_dir = "tf_vuv_ckpt_D64_D32_D1"
n_data_process = 4
test_mode = True

f0_path = os.path.abspath("C:/train/vuvdetect_train_f0_marcoclean.mmap")
feature_path = os.path.abspath("C:/train/vuvdetect_train_feature_marcoclean.mmap")
  ...
  while True:
    ...
    with memory_f0.get_lock():
      with memory_feature.get_lock():
        np.frombuffer(memory_f0.get_obj(), dtype=np.bool).reshape(batch_size)[:], np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_feature)[:] = fetch_data()
...
if __name__ == "__main__":
  ...
  if test_mode:
    ...
    p_target = tf.placeholder(dtype=tf_dtype, shape=[None, 1], name="data_target")
  else:
    p_input = tf.placeholder(dtype=tf_dtype, shape=[batch_size, n_feature], name="data_input")
    p_target = tf.placeholder(dtype=tf_dtype, shape=[batch_size, 1], name="data_target")
  v_logits, v_net = revoice.tf_model.hubbleVUVNetwork2(p_input, isTrain=not test_mode, reuse=False, scopeName="net_hubble_vuv")
  ...
  if test_mode:
    ...
    tracker = revoice.tf_model.HubbleVUVTrajectory()
    path = tracker(vuv_list)
    pl.figure()
    ...
    for i in range(n_data_process * 2):
      memory_pool.append((mp.Array(ctypes.c_bool, batch_size), mp.Array(ctypes.c_float, batch_size * n_feature)))
      memory_queue.put(i)
      del i
    ...
    def fetch_mem_data():
      ...
      with memory_f0.get_lock():
        with memory_feature.get_lock():
          out = np.frombuffer(memory_f0.get_obj(), dtype=np.bool).reshape(batch_size, 1).astype(np.float32), np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_feature).copy()
