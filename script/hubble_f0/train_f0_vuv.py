...
path_checkpoint_dir = "tf_vuv_ckpt_D128_G80_D65"
n_data_process = 4
test_mode = True
...
def fetch_data():
  ...
  f0_map_list = np.zeros((batch_size, n_hop_per_sample, n_band + 1), dtype=np.float32)
  for i_batch in range(batch_size):
    for i_hop in range(n_hop_per_sample):
      if f0_list[i_batch, i_hop] <= 0.0:
        f0_map_list[i_batch, i_hop, -1] = 1.0
      else:
        f0_map_list[i_batch, i_hop, np.argmin(np.abs(f0_list[i_batch, i_hop] - band_freq_list))] = 1.0
  ...
  while True:
    ...
    with memory_f0.get_lock():
      with memory_feature.get_lock():
        np.frombuffer(memory_f0.get_obj(), dtype=np.bool).reshape(batch_size, n_hop_per_sample, n_band + 1)[:], np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_feature)[:] = fetch_data()
...
if __name__ == "__main__":
  ...
  if test_mode:
    ...
    p_target = tf.placeholder(dtype=tf_dtype, shape=[None, None, n_band + 1], name="data_target")
  else:
    p_input = tf.placeholder(dtype=tf_dtype, shape=[batch_size, n_hop_per_sample, n_feature], name="data_input")
    p_target = tf.placeholder(dtype=tf_dtype, shape=[batch_size, n_hop_per_sample, n_band + 1], name="data_target")
  v_logits, v_net = revoice.tf_model.hubbleF0Network3(p_input, isTrain=not test_mode, reuse=False, scopeName="net_hubble_vuv")
  ...
  if test_mode:
    ...
    for i in range(n_data_process * 2):
      memory_pool.append((mp.Array(ctypes.c_bool, batch_size * n_hop_per_sample * (n_band + 1)), mp.Array(ctypes.c_float, batch_size * n_hop_per_sample * n_feature)))
      memory_queue.put(i)
      del i
    ...
    def fetch_mem_data():
      ...
      with memory_f0.get_lock():
        with memory_feature.get_lock():
          out = np.frombuffer(memory_f0.get_obj(), dtype=np.bool).reshape(batch_size, n_hop_per_sample, n_band + 1).astype(np.float32), np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_feature).copy()
    ...
    for i_epoch in range(n_epoch):
      ...
      '''
      pl.figure()
      pl.imshow(input_list.reshape(batch_size, n_hop_per_sample, n_band, 3)[512, :, :, 1].T, cmap='plasma', interpolation='nearest', aspect='auto', origin='lower')
      pl.plot(np.argmax(target_list[512], axis=1), "cx")
      pl.figure()
      pl.imshow(target_list.reshape(batch_size, n_hop_per_sample, n_band + 1)[512].T, cmap='plasma', interpolation='nearest', aspect='auto', origin='lower')
      pl.plot(np.argmax(target_list[512], axis=1), "cx")
      pl.show()
      '''
