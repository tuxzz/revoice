...
path_checkpoint_dir = "tf_vuv_ckpt"
n_data_process = 4
test_mode = True
...
def fetch_data():
  ...
  f0_map_list = np.zeros((batch_size, n_hop_per_sample, n_band), dtype=np.float32)
  ...
  while True:
    ...
    with memory_f0.get_lock():
      with memory_feature.get_lock():
        np.frombuffer(memory_f0.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_band)[:], np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_feature)[:] = fetch_data()
...
if __name__ == "__main__":
  ...
  p_target = tf.placeholder(dtype=tf_dtype, shape=[None, None, n_band], name="data_target")
  v_net = revoice.tf_model.hubbleF0Network(p_input, isTrain=not test_mode, reuse=False, scopeName="net_hubble_f0")
  v_loss = revoice.tf_model.costFunction(p_target, v_net)
  ...
  if test_mode:
    ...
    for i in range(n_data_process * 2):
      memory_pool.append((mp.Array(ctypes.c_float, batch_size * n_hop_per_sample * n_band), mp.Array(ctypes.c_float, batch_size * n_hop_per_sample * n_feature)))
      memory_queue.put(i)
      del i
    ...
    def fetch_mem_data():
      ...
      with memory_f0.get_lock():
        with memory_feature.get_lock():
          out = np.frombuffer(memory_f0.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_band).copy(), np.frombuffer(memory_feature.get_obj(), dtype=np.float32).reshape(batch_size, n_hop_per_sample, n_feature).copy()
    ...
    for i_epoch in range(n_epoch):
      ...
      pl.plot(np.argmax(target_list[0], axis=1))
      pl.figure()
      pl.imshow(target_list[0].reshape(n_hop_per_sample, n_band).T, cmap='plasma', interpolation='nearest', aspect='auto', origin='lower')
      pl.plot(np.argmax(target_list[0], axis=1))
      pl.show()
      '''
